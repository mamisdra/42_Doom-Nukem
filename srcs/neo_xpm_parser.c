...
/*   Updated: 2020/03/08 18:58:55 by mamisdra         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		read_line(char *buff, int *token, t_tex_data *data, t_texture *tex)
{
...
void		read_tex(t_texture *tex)
{
	...
	if ((fd = open(tex->name, O_RDONLY)) < 0)
		ft_exit("open texture file error");
	while ((ret = get_next_line(fd, &buff)) == 1)
	{
		read_line(buff, &token, &data, tex);
		free(buff);
	}
	free(data.trad);
	ret = -1;
