...
/*   Updated: 2020/02/22 17:13:43 by mamisdra         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		set_mat(t_mat4x4 *m, t_vec3d pos, t_vec3d newforward, t_vec3d newup)
{
	m->m[0][3] = 0.0f;
	m->m[1][0] = newup.x;
	m->m[1][1] = newup.y;
...
t_mat4x4	mat_pointat(t_vec3d pos, t_vec3d target, t_vec3d up)
{
	...
	m.m[0][1] = newright.y;
	m.m[0][2] = newright.z;
	set_mat(&m, pos, newforward, newup);
	return (m);
}
...
t_mat4x4	mat_quick_invert(t_mat4x4 m)
{
	...
	m1.m[2][2] = m.m[2][2];
	m1.m[2][3] = 0.0f;
	m1.m[3][0] = -(m.m[3][0] * m1.m[0][0] + m.m[3][1] * m1.m[1][0] +\
	m.m[3][2] * m1.m[2][0]);
	m1.m[3][1] = -(m.m[3][0] * m1.m[0][1] + m.m[3][1] * m1.m[1][1] +\
	m.m[3][2] * m1.m[2][1]);
	m1.m[3][2] = -(m.m[3][0] * m1.m[0][2] + m.m[3][1] * m1.m[1][2] +\
	m.m[3][2] * m1.m[2][2]);
	m1.m[3][3] = 1.0f;
	return (m1);
}
