...
{
	var.dy1 = (int)(tri.p[1].y - tri.p[0].y);
	var.dx1 = (int)(tri.p[1].x - tri.p[0].x);
	var.dv1 = tri.t[1].v - tri.t[0].v;
	var.du1 = tri.t[1].u - tri.t[0].u;
	var.dw1 = tri.t[1].w - tri.t[0].w;
	var.dy2 = (int)(tri.p[2].y - tri.p[0].y);
	var.dx2 = (int)(tri.p[2].x - tri.p[0].x);
...
{
	if (var.dy1)
		var.dax_step = (float)var.dx1 / (float)abs(var.dy1);
	if (var.dy2)
		var.dbx_step = (float)var.dx2 / (float)abs(var.dy2);
	if (var.dy1)
